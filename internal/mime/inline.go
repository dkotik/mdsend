...
func splitAttachments(s string, attachments []cachedAttachment) (normal []cachedAttachment, inline []cachedAttachment) {
	cids := slices.Collect(eachSourceWithContentID([]byte(s)))
	for _, a := range attachments {
		if a.ContentID == "" {
			normal = append(normal, a)
		} else if slices.Contains(cids, a.ContentID) {
